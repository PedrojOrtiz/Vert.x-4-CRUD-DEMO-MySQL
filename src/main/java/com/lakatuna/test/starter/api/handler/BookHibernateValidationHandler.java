...
public class BookHibernateValidationHandler {

  private final Vertx vertx;

  public BookHibernateValidationHandler(Vertx vertx) {
    this.vertx = vertx;
  }
  ...
  /**
   * Build logicDelete one book request validation
   *
   * @return ValidationHandler
   */
  public ValidationHandler logicDelete() {
    final SchemaParser schemaParser = buildSchemaParser();
  ...
  private SchemaParser buildSchemaParser() {
    return SchemaParser.createDraft7SchemaParser(SchemaRouter.create(vertx.getDelegate(), new SchemaRouterOptions()));
  }
